...
#include "FreeRTOS.h"
#include "cmsis_os2.h"
extern  osSemaphoreId_t  RS485RXHandle;
static volatile uint8_t g_ht_interrupt_locked = 0;
...
void MX_USART2_UART_Init(void)
{
  ...
  __HAL_DMA_DISABLE_IT(&hdma_usart2_rx, DMA_IT_HT);                                                 // ??DMA????
  HAL_UARTEx_ReceiveToIdle_DMA(&huart2, (uint8_t *)uart_rx_dma_buffer, sizeof(uart_rx_dma_buffer)); // ??DMA????
...
void MX_USART3_UART_Init(void)
{
  ...
  __HAL_DMA_DISABLE_IT(&hdma_usart3_rx, DMA_IT_HT);
	Lock_HT_Interrupt(&hdma_usart3_rx);
...
{
    g_ht_interrupt_locked = 1;
    
    // 直接写入CCR寄存器，确保HTIE位为0
    hdma->Instance->CCR &= ~DMA_CCR_HTIE;
    ...
    // 再次确认写入成功
    if(hdma->Instance->CCR & DMA_CCR_HTIE)
    {
        // 如果还有，强制清除
        hdma->Instance->CCR &= ~DMA_CCR_HTIE;
    }
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
  ...
  if (huart == &huart2)
  {
    ...
    HAL_UARTEx_ReceiveToIdle_DMA(&huart2, (uint8_t *)uart_rx_dma_buffer, sizeof(uart_rx_dma_buffer));
   
  }
  if (huart == &huart3) // RS485
  {
