#include "RS485.h"
#include "stm32f407xx.h"
#include "stm32f4xx_hal_gpio.h"
#include "stm32f4xx_hal_uart.h"
#include "usart.h"
#include <stdint.h>
#include "stdio.h"
#include "usart.h"
#include <stdint.h>
#include "time.h"
#include "FreeRTOS.h"
#include "MYDWT.h"
#include "task.h"
#include "usart_lcd.h"

float velocity_arr[1] = {0};
uint16_t  weiyi_flag = 0;
// 应答数据定义
int Slave1_ACK = 0;
int Slave2_ACK = 0;
int Slave3_ACK = 0;
int Slave4_ACK = 0;
int Master_ACK = 0;
int Slave1_ACK_ERR_CONT = 0;
int Slave2_ACK_ERR_CONT = 0;
int Slave3_ACK_ERR_CONT = 0;
int Slave4_ACK_ERR_CONT = 0;
float target_velocity_1[1] = {0};
float target_velocity_2[1] = {0};
float target_velocity_3[1] = {0};
float target_velocity_4[1] = {0};
volatile int IQ_ERROR = 0;

//用来管理每次485的发送类型
volatile uint16_t  current_Send_frame_type      = DATA_Frame;
//用来管理每次485的接收类型
volatile uint16_t  current_Receive_frame_type   = 0;
volatile uint16_t  Receive_frame_4ID[4]  = {0};//四驱动板
volatile uint16_t  Receive_frame_2ID[4]  = {0};//二驱动板


// CRC表

static const uint8_t crc8_table[256] = {
    };

/**
 * @brief CRC-8 校验函数 (查表法)
 * @param data 待校验数据指针
 * @param len 数据长度(字节)
 * @return CRC8校验值
 * @note 多项式: x^8 + x^2 + x + 1 (0x07), 初始值: 0xFF
 */
uint8_t crc8_calculate(const uint8_t *data, size_t len)
{
    uint8_t crc = 0xFF; // 初始值

    for (size_t i = 0; i < len; i++)
    {
        crc = crc8_table[crc ^ data[i]];
    }
    return crc;
}

void test_crc8(void)
{
    uint8_t test_data[] = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39};
    uint8_t crc = crc8_calculate(test_data, sizeof(test_data));
}

// 获取速度值数据帧 (支持float类型)
void Slave_SEND_Frame(uint16_t ID, float *float_arr, int size, uint16_t frame_type)
{
    clear_ACK(); // 发送前清空
    // 帧结构: [包头2] [ID信息2] [长度1] [数据(N*4)] [CRC8 1] [包尾1]
    // 每个float拆分为2个uint16_t, 共 N*4 字节

    int data_bytes = 4 * size;      // 数据部分字节数 (float占4字节)
    int total_words = 2 * size + 4; // 总数组长度(包含帧头、长度、CRC、帧尾)

    uint16_t rs485_buf[total_words];
    uint8_t *byte_buf = (uint8_t *)rs485_buf; // 用于CRC计算的指针

    // 填充帧头
    rs485_buf[0] = (HEAD << 8) | frame_type; // 包头 + 功能码
    #ifdef MY_ID
    rs485_buf[1] = (MY_ID << 8) | ID;        // 主机ID + 从机ID
    #endif 
    
    
    #ifdef MY_ID_diantiao
    rs485_buf[1] = (MY_ID_diantiao << 8) | ID;        // 主机ID + 从机ID
    #endif 
    
    
    // 填充数据长度 (字节数)
    rs485_buf[2] = data_bytes;

    // 填充数据: float → int32 → 拆分为两个uint16_t
    for (int i = 0; i < size; i++)
    {
        union
        {
            float f;
            uint32_t u32;
        } f2u = {.f = float_arr[i]};

        rs485_buf[3 + 2 * i] = (f2u.u32 >> 16) & 0xFFFF; // 高16位
        rs485_buf[3 + 2 * i + 1] = f2u.u32 & 0xFFFF;     // 低16位
    }

    // 计算CRC8 (覆盖帧头、长度、数据)
    int crc_length = 2 * (2 + 1 + 2 * size); // 前3个word + 数据部分
    uint8_t crc8 = crc8_calculate(byte_buf, crc_length);

    // 填充CRC8和帧尾
    rs485_buf[3 + 2 * size] = (crc8 << 8) | END;

    RS485_SendBytes(rs485_buf, sizeof(rs485_buf), 1); // RS485发送
}

// 主机发送数据帧 (支持float类型)
void MASTER_SEND_Frame(uint16_t ID, float *float_arr, int size  ,uint16_t frame_type)
{

    // 帧结构: [包头2] [ID信息2] [长度1] [数据(N*4)] [CRC8 1] [包尾1]
    // 每个float拆分为2个uint16_t, 共 N*4 字节

    int data_bytes = 4 * size;      // 数据部分字节数 (float占4字节)
    int total_words = 2 * size + 4; // 总数组长度(包含帧头、长度、CRC、帧尾)

    uint16_t rs485_buf[total_words];
    uint8_t *byte_buf = (uint8_t *)rs485_buf; // 用于CRC计算的指针

    // 填充帧头
    rs485_buf[0] = (HEAD << 8) | frame_type; // 包头 + 功能码
    rs485_buf[1] = (MASTER_ID << 8) | ID;    // 主机ID + 从机ID

    // 填充数据长度 (字节数)
    rs485_buf[2] = data_bytes;

    // 填充数据: float → int32 → 拆分为两个uint16_t
    for (int i = 0; i < size; i++)
    {
        union
        {
            float f;
            uint32_t u32;
        } f2u = {.f = float_arr[i]};

        rs485_buf[3 + 2 * i] = (f2u.u32 >> 16) & 0xFFFF; // 高16位
        rs485_buf[3 + 2 * i + 1] = f2u.u32 & 0xFFFF;     // 低16位
    }

    // 计算CRC8 (覆盖帧头、长度、数据)
    int crc_length = 2 * (2 + 1 + 2 * size); // 前3个word + 数据部分
    uint8_t crc8 = crc8_calculate(byte_buf, crc_length);

    // 填充CRC8和帧尾
    rs485_buf[3 + 2 * size] = (crc8 << 8) | END;

    RS485_SendBytes(rs485_buf, sizeof(rs485_buf), 1); // RS485发送
}









// 请求从机发送数据命令
void MASTER_ENABLE_Slave_TX(uint16_t ID, uint16_t command)
{
    uint16_t rs485_buf[4];
    rs485_buf[0] = (HEAD << 8) | Command_Frame; // 包头 + 功能码
    rs485_buf[1] = (MASTER_ID << 8) | ID;       // 主机ID + 从机ID
    rs485_buf[2] = command;                     // 命令长度 1字节
    rs485_buf[3] = (0x00 << 8) | END;
    RS485_SendBytes(rs485_buf, sizeof(rs485_buf), 0); // RS485发送
}


/**
 * 精确的us级延时
 * @brief 使用循环计数实现微秒级延时
 * @param us: 延时的微秒数
 * @note 需要根据主频调整DELAY_US_COUNT
 */
void delay_us(uint32_t us)
{
    // 计算循环次数（在168MHz下经验值）
    // 每个循环大约需要9个时钟周期
    const uint32_t DELAY_US_COUNT = SystemCoreClock / 1000000 / 9;
    
    for(uint32_t i = 0; i < us; i++) {
        for(uint32_t j = 0; j < DELAY_US_COUNT; j++) {
            __asm__ volatile ("nop");
        }
    }
}





// 485发送数据    数组地址，长度，是否需要应答信号，自动重传次数

   void RS485_SendBytes(uint16_t *buf, uint16_t len, int count_SET)
{

    RS485_TX_EN;
    delay_us(2);
    HAL_UART_Transmit_DMA(&huart6, (uint8_t *)buf, len);
    while (huart6.gState != HAL_UART_STATE_READY); // 等待发送完成
    RS485_RX_EN;
    delay_us(2);
    uint32_t WAIT_ACK_Start = TIM2->CNT;

    while (TIM2->CNT <= WAIT_ACK_Start + 70) // 100us内没有收到应答数据等待
    {

        taskYIELD();
    }

}

// 485接收数据处理
void Analyze_RS485_data(void)
{

    // 01 AA 01 55 10
    // 00 00 00 00 00
    // 00 00 00 00 00
    // 00 00 00 00 00
    // 00 00 BB 1A   数据帧例子

    /*
    3	AA	55  1 	BB  命令帧
    */
    if (RS485_flag)
    {
        uint8_t FRAME_type = 0x00;
        FRAME_type = RS485_dma_buffer[0];  // 获取报文类型
//				printf("0x%x\n",FRAME_type);
        uint8_t ID1 = RS485_dma_buffer[3]; // 发送方ID
        uint8_t ID2 = RS485_dma_buffer[2]; // 接收方ID

        //		printf("%02X\n",ID2);

        #ifdef  MY_ID
         if (ID2 != MY_ID)
        {
            RS485_flag = 0;
            return;
        }
        #endif 
        #ifdef MY_ID_diantiao
        if(ID2 != MY_ID_diantiao )
        {
            RS485_flag = 0;
            return; 
        }
        #endif         


        if (FRAME_type == DATA_Frame ||  FRAME_type == Distance_Frame ) // 这是一个数据报文，让我看看是什么数据
        {
            if (RS485_dma_buffer[1] == 0xAA) // 包头没问题
            {

                uint8_t data_length = RS485_dma_buffer[4] / 4; // float数据个数，字节数
                //        printf("%d\n",data_length);
                float float_ARRY[data_length];
                uint16_t gao_16 = 0;
                uint16_t di_16 = 0;
                for (int i = 0; i < data_length; i++) // 遍历每一个浮点数
                {
                    gao_16 = RS485_dma_buffer[6 + 4 * i + 1] << 8 | RS485_dma_buffer[6 + 4 * i];    // 高16位
                    di_16 = RS485_dma_buffer[5 + 4 * i + 3] << 8 | RS485_dma_buffer[5 + 4 * i + 2]; // 低16位
                    uint32_t combined = ((uint32_t)gao_16 << 16) | di_16;
                    float_ARRY[i] = *((float *)&combined);
                }
                uint8_t crc_Frme = RS485_dma_buffer[5 + data_length * 4 + 2];            // CRC8校验值
                uint8_t crc_cal = crc8_calculate(RS485_dma_buffer, 6 + data_length * 4); // 计算CRC8校验值

                if (crc_Frme == crc_cal) // CRC校验通过
                {
                    if(FRAME_type  ==  DATA_Frame)
                    {
                        // 接收本次速度数据
                        switch (ID2)
                        {
                        case Slave1_ID:
                            velocity_arr[0] = float_ARRY[0];
                            break;
                        case Slave2_ID:
                            velocity_arr[0] = float_ARRY[0];
                            break;
                        case Slave3_ID:
                            velocity_arr[0] = float_ARRY[0];
                            break;
                        case Slave4_ID:
                            velocity_arr[0] = float_ARRY[0];
                            break;
                        }
                    }else if (FRAME_type  ==  Distance_Frame)
                    {
                        // 接收本次位移数据
                        switch (ID2)
                        {
                         case Slave1_ID:
                            velocity_arr[0] = float_ARRY[0];        //如果发送的值是0，则为速度模式；1为距离闭环模式
                            break;
                        case Slave2_ID:
                            velocity_arr[0] = float_ARRY[0];
                            break;
                        case Slave3_ID:
                            velocity_arr[0] = float_ARRY[0];
                            break;
                        case Slave4_ID:
                            velocity_arr[0] = float_ARRY[0];
                            break;                       
                        }
                    }
                    
                    RS485_flag = 0;    // 置零代表没有新数据了
                    if (IQ_ERROR == 0) // 没有错误
                    {
                        send_ACKback(ID2, ID1); // 发送应答位回去
                    }
                    else
                    {
                        send_ERRORback(ID1, ID2);
                        IQ_ERROR = 0;
                    }
                }
                else
                {
                    RS485_flag = 0; // 置零代表没有新数据了
                }
            }
            else
            {
                RS485_flag = 0; // 置零代表没有新数据了
            }
        }
        else // 是命令帧我看看是啥命令
        {
            switch (RS485_dma_buffer[0])
            {
            //////////////////////////////////////////////////////////
            case ACK_Frame:
                Analyze_ACKfrme_data(ID1, ID2);
                break;
            //////////////////////////////////////////////////////////
            case VElocity:
                Slave_SEND_Frame(ID1, velocity_arr, sizeof(velocity_arr) ,DATA_Frame);  
                break;
			//////////////////////////////////////////////////////////		
			case Command_Frame: // 位移结束，发送完成确认帧
              if (weiyi_flag  ) {Slave_SEND_Frame(MASTER_ID, NULL, 0, DISTANCE_OK_Frame);
              }else{Slave_SEND_Frame(MASTER_ID, NULL, 0, DISTANCE_Uncomplete_Frame);}break;
            //////////////////////////////////////////////////////////
            case ERROR_Frame: LCD_red();Analyze_ACKfrme_data(ID1,ID2);   break;
            //////////////////////////////////////////////////////////
            case  DISTANCE_OK_Frame:

            #ifdef MY_ID_diantiao
                switch (ID1)
                {
                case Slave1_ID:Receive_frame_2ID[0] = 1;
                    break;
                case Slave2_ID:Receive_frame_2ID[1] = 1;
                    break;
                }
            #endif 
            #ifdef MY_ID
                switch (ID1)
                {
                case Slave1_ID:Receive_frame_4ID[0] = 1;
                    break;
                case Slave2_ID:Receive_frame_4ID[1] = 1;
                    break;
                case Slave3_ID:Receive_frame_4ID[2] = 1;
                    break;
                case Slave4_ID:Receive_frame_4ID[3] = 1;
                    break;
                }
            #endif 
            
            default:
                break;
            }
                
                
        }
    }
    RS485_flag = 0;
}

// 刷新ACK信号
void clear_ACK(void)
{
    Master_ACK = wait_ack;
    Slave1_ACK = wait_ack;
    Slave2_ACK = wait_ack;
    Slave3_ACK = wait_ack;
    Slave4_ACK = wait_ack;
}

// 分析应答帧数据
void Analyze_ACKfrme_data(uint8_t SEND_ID1, uint8_t receive_ID2)
{
    #ifdef MY_ID
    if (receive_ID2 == MY_ID)
    {
        //				printf("9\n");
        switch (SEND_ID1)
        {
        case MASTER_ID:
            Master_ACK = get_ack;
            break;

        case Slave1_ID:
            Slave1_ACK = get_ack;
            break;

        case Slave2_ID:
            Slave2_ACK = get_ack;
            break;

        case Slave3_ID:
            Slave3_ACK = get_ack;
            break;

        case Slave4_ID:
            Slave4_ACK = get_ack;
            break;
        }
    }
    #endif
    #ifdef MY_ID_diantiao
    if (receive_ID2 == MY_ID_diantiao)
    {
        //				printf("9\n");
        switch (SEND_ID1)
        {
        case MASTER_ID:
            Master_ACK = get_ack;
            break;

        case Slave1_ID:
            Slave1_ACK = get_ack;
            break;

        case Slave2_ID:
            Slave2_ACK = get_ack;
            break;

        case Slave3_ID:
            Slave3_ACK = get_ack;
            break;

        case Slave4_ID:
            Slave4_ACK = get_ack;
            break;
        }
    }
    #endif

}

// 发送应答位回去
void send_ACKback(uint8_t SEND_ID1, uint8_t receive_ID2) // 收到了别人给我的报文
{
    uint16_t rs485_buf[3];
    rs485_buf[0] = (HEAD << 8) | ACK_Frame;       // 包头 + 功能码
    rs485_buf[1] = (receive_ID2 << 8) | SEND_ID1; // 高位发送方低位接收方 ，我再发给别人
    rs485_buf[2] = END;
    RS485_SendBytes(rs485_buf, sizeof(rs485_buf), 0);
}

// 发送错误帧回去
void send_ERRORback(uint8_t SEND_ID1, uint8_t receive_ID2) // 收到了别人给我的报文
{
    uint16_t rs485_buf[3];
    rs485_buf[0] = (HEAD << 8) | ERROR_Frame;     // 包头 + 功能码
    rs485_buf[1] = (receive_ID2 << 8) | SEND_ID1; // 高位发送方低位接收方 ，我再发给别人
    rs485_buf[2] = END;
    RS485_SendBytes(rs485_buf, sizeof(rs485_buf), 0);
}

// 发送数据的的次序，依次对应从机ID次序
void RS485_SET_MOTEOR_SPEED(float *data_1, float *data_2, float *data_3, float *data_4)
{

    MASTER_SEND_Frame(Slave1_ID, data_1, 1,DATA_Frame);
    Analyze_RS485_data();
    MASTER_SEND_Frame(Slave2_ID, data_1, 1,DATA_Frame);
    Analyze_RS485_data();
    MASTER_SEND_Frame(Slave3_ID, data_1, 1,DATA_Frame);
    Analyze_RS485_data();
    MASTER_SEND_Frame(Slave4_ID, data_1, 1,DATA_Frame);
    Analyze_RS485_data();

    if ((Slave1_ACK & Slave2_ACK & Slave3_ACK & Slave4_ACK) == get_ack)
    {
        clear_ACK(); // 清除ACK信号
    }
    else
    {
        if (Slave1_ACK == wait_ack)
            Slave1_ACK_ERR_CONT++;
        if (Slave2_ACK == wait_ack)
            Slave2_ACK_ERR_CONT++;
        if (Slave3_ACK == wait_ack)
            Slave3_ACK_ERR_CONT++;
        if (Slave4_ACK == wait_ack)
            Slave4_ACK_ERR_CONT++;
        clear_ACK();
    }
}


