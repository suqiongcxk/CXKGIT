...
float target_velocity_3 = 0;
float target_velocity_4 = 0;
//应答数据定义
int  Slave1_ACK   = 0;
int  Slave2_ACK   = 0;
int  Slave3_ACK   = 0;
...
//        printf("%d\n",data_length);
        ...
        if(crc_Frme == crc_cal )  //CRC校验通过
        {
            ...
            switch ( ID2 )
            {
                case Slave1_ID :target_velocity_1 = float_ARRY[0]; break ;
                case Slave2_ID :target_velocity_2 = float_ARRY[0]; break ;
                case Slave3_ID :target_velocity_3 = float_ARRY[0]; break ;
                case Slave4_ID :target_velocity_4 = float_ARRY[0]; break ;            
            }
            RS485_flag = 0; //置零代表没有新数据了
//						for  (int i = 0; i<data_length ;i++)
//						{
//								printf("%f ", float_ARRY[i]);
//						}
//						while(1){};
            send_ACKback(ID1,ID2); //发送应答位回去 
        }
        else
        {
            printf("CRC_ERROR\n");
    ...
    else  //是命令帧我看看是啥命令
    {
        ...
        {
            case ACK_Frame:RS485_flag = 0; Analyze_ACKfrme_data(ID1,ID2);   break;
         
            case VElocity : RS485_flag = 0;Slave_SEND_DATA(ID1,velocity_arr,sizeof(velocity_arr));  break;
        }
    }
    }
    RS485_flag = 0;
